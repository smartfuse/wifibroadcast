...
BASE_IMAGE_URL="http://downloads.raspberrypi.org/raspbian/images/raspbian-2017-09-08"
BASE_IMAGE="2017-09-07-raspbian-stretch"
...
function patch_rpi_image {
	...
	mkdir -p "$MNT_DIR"
	# rootfs
	mountpoint -q "$MNT_DIR" || sudo mount "$IMAGE_FILE" -o loop,offset=48234496,rw "$MNT_DIR"
	# boot
	mountpoint -q "$MNT_DIR/boot" || sudo mount "$IMAGE_FILE" -o loop,offset=4194304,rw "$MNT_DIR/boot"
